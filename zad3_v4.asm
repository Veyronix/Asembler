...
KOLEJNE_ZNAKI:
	...
	jz NAPRZOD
	cmp ds:[bufor+si],117d ;u up
	jz DO_GORY
	cmp ds:[bufor+si],100d ;d down
	jz OPUSC
	jmp KONIEC_KOLEJNE_ZNAKI
OBROT:
	inc si
	cmp ds:[bufor+si],32d;spacja
	jz OBROT
...
;AKTUALIZACJA DANYCH NA STOSIE CZYLI Px itd  
	...
	fistp ds:[smietnik] ;USUNIECIE DANYCH
	fistp ds:[smietnik]
	fldz
	fadd st(0),st(2)
	fldz
...
ZAMALUJ_PUNKT PROC ; wspolrzedne punktu brane z ds:[tmpx] i ds:[tmpy]
	...
	shl ax,1
	add di,ax ;DI =320*y
	add di,word ptr ds:[tmpx] ; DI=320*y+x
	mov al,byte ptr ds:[kolor]
	mov byte ptr es:[di],al ;wyslanie do ES:DI = 0A000:(320*y+x) koloru kolor
petla:
	;in al, 60h
	;cmp al,1
	;jne petla
